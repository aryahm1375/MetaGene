...
def test(model, data, labels,fold, test):
    ...
    with torch.no_grad():
        ...
        if test:
            with open(f'{target_data}_Transformer/{fold}_{global_batch}_{GCN_size}_{str(alpha_rate).replace(".","_")}_{target_data}.txt', 'a') as the_file:
...
class GCN(nn.Module):
    ...
    def __init__(self, num_features):
        super(GCN, self).__init__()
        self.embedding = nn.Linear(695, num_features)
        self.transformer_layer = nn.TransformerEncoderLayer(d_model=num_features, nhead=1)
